...
function Instance(portPool, options) {
  ...
  this.notices = options.impressNotices || typeof options.impressNotices == 'undefined';
  this.warnings = options.impressWarnings || typeof options.impressWarnings == 'undefined';
  this.resourcesLogging = options.impressResourcesLogging;
...
Instance.prototype = {
  ...
  _initReadyPromise: function() {
    ...
    this.readyPromise = function(fn) {
      if (self._killed) {
        logger.error('Error: Add ready listener for killed phantom instance');
        return noop;
      }
      if (this._started) {
        try {
  ...
  _getExecArgs: function() {
    ...
    if (this.resourcesLogging) {
      builder.push('--resources-logging');
    }
  ...
  _exec: function() {
    ...
    if (this._process) {
      logger.warn('Warn: Phantom process already executed');
      return;
    }
    ...
    function errorHandler(error) {
      ...
      logger.error('Error: Phantom process error', error || '');
      self._kill();
    }
    ...
    function closeHandler(code) {
      ...
      self._cancelExecTimeout();
      logger.log('Info: Phantom process closed');
      if (self._killed) {
        return;
      }
      self._kill();
    ...
    function ready() {
      ...
      self._started = true;
      logger.log('Ok: phantom process started in time', Date.now() - time, 'ms');
      self._em.emit('ready');
    }
    ...
    try {
      ...
      stdout.on('data', function(data) {
        ...
        logger.log('Info: Phantom process output:', data);
  ...
  _kill: function() {
    ...
    if (!killed) {
      logger.log('Info: Phantom process kill on port', this.serverPort);
    }
    ...
    try {
      ...
      logger.error('Error: Phantom kill process error', e);
    }
    this._process = null;
    this._killed = true;
