...
Application.prototype = {
  ...
  _getParsedConfig: function(/* ...configs */) {
      ...
      config.forEach(function(config) {
        ...
        try {
          ...
          if ( !(optional && error instanceof Error && error.code == 'ENOENT') ) {
            logger.error('Could not parse config file "'+config+'"', error);
          }
        }
      });
    });
    ...
    function _merge(config) {
      if (config && typeof config == 'object' && !Array.isArray(config)) {
        ...
        logger.warn('Invalid config data:', JSON.stringify(config));
      }
    }
    return result;
  },
  ...
  performConfigFile: function(fileName, optional) {
    ...
    try {
      ...
      logger.error('Error in configuration file', error);
    }
  },
