...
function ImpressInstance(deferred, options) {
  ...
  this.reportNotices = options.impressReportNotices;
  this.excludeReport = typeof options.excludeImpressReport == 'undefined' || options.excludeImpressReport;
...
ImpressInstance.prototype = {
  ...
  _getCommandExecString: function(base64EncodedUrl) {
    ...
    if (this.reportNotices) {
      builder.push('--report-notices');
    }
  ...
  run: function() {
    ...
    function finish(err, content, warn) {
      stopTimeout();
      content = self._performImpressReport(content);
  ...
  _performImpressReport: function(content) {
    var
      match = content.match(/<script type="application\/impress-report\+json"><!\[CDATA\[([\S\s]*?)\]\]><\/script>/i),
      report,
    ...
    if (match) {
      try {
        ...
        if (report.warnings && report.warnings.length) {
          console.warn('IMPRESS WARNINGS for page "' + (report.url || url) + '": ', JSON.stringify(report.warnings));
        }
      }
      catch(e) {
        console.warn('WARNING page "' + url + '" could not be found valid report', e);
      }
      if (this.excludeReport) {
  ...
  _invoke: function() {
    ...
    function errorHandler(error) {
      if (error) {
        errorDataBuilder.push(error);
      }
      finish(getErrorData() || getResultData(), getResultData());
    }

    function getErrorData() {
      return errorDataBuilder.join('');
    }

    function getResultData() {
      return resultDataBuilder.join('');
    }
        ...
        child.on('close', function (code) {
          switch(code) {
            ...
            case PAGE_NOT_FOUND_EXIT_CODE:
              finish(new PageNotFoundError(), getResultData(), getErrorData());
              break;
