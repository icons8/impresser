...
function Instance(portPool, options) {
  ...
  this._htmlFilter = new HtmlFilter(options);
}
...
Instance.prototype = {
  ...
  _addBlockedResources: function(resources) {
    if (Array.isArray(resources)) {
      ...
      if (Array.isArray(resources)) {
        this._addBlockedResources(resources);
      }
    }
  },
  ...
  _getCommandExecString: function() {
    ...
    if (this._blockedResources.length > 0) {
      builder.push('--blocked-resources');
      Array.prototype.push.apply(
        builder,
        this._blockedResources.map(base64encode)
      );
  ...
  _apply: function() {
    ...
    function done(result) {
      ...
      result.content = self._htmlFilter.apply(result.content);
