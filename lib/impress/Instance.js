...
Instance.prototype = {
  ...
  _apply: function() {
    ...
    try {
      req = http.request(requestOptions, function(res) {
        ...
        res.on('end', function() {
          ...
          if (error || (error = self._validateContentAndGetValidationError(result.content))) {
    ...
    this._impressRequestTimeoutId = setTimeout(
      function() {
        console.error('Impress request timeout', self.timeout);
        self._abortImpressRequest();
      },
      this.timeout
    );
  ...
  _abortImpressRequest: function() {
    ...
    this._impressRequestTimeoutId && clearTimeout(this._impressRequestTimeoutId);
    this._impressRequestTimeoutId = null;
  },
