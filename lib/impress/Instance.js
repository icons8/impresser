...
function Instance(portPool, options, htmlFilters) {
  ...
  this.ttl = options.phantomTtl || DEFAULT_INSTANCE_TTL;
...
Instance.prototype = {
  ...
  _getCommandExecString: function() {
    ...
    builder.push(
      ...
      '--timeout ' + (this.attemptTimeout - IMPRESS_ATTEMPT_TIMEOUT_SAFETY_TIME)
    );
  ...
  _invoke: function() {
    ...
    try {
      child = exec(this._getCommandExecString());
  ...
  _resetTtlTimeout: function() {
    ...
    this._ttlTimeoutId = setTimeout(
      ...
      this.ttl + Math.ceil(Math.random() * this.ttl * TTL_SPREAD_FACTOR)
    )
  },
