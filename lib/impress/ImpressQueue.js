var
  ImpressParallel = require('./ImpressParallel'),
  ImpressDeferred = require('./ImpressDeferred')
  ;

module.exports = ImpressQueue;

function ImpressQueue(options) {
  options = options || {};

  this.options = options;
  this.parallel = new ImpressParallel(options);
  this.maxSize = options.maxQueue || this.parallel.maxSize * 30;

  this.queue = [];
  this._registerPoolWatcher();
}

ImpressQueue.prototype = {

  hasLimit: function() {
    return this.queue.length >= this.maxSize;
  },

  isEmpty: function() {
    return this.queue.length == 0;
  },

  add: function(url) {
    var
      deferred,
      queue = this.queue,
      parallel = this.parallel.parallel,
      index
      ;

    for (index = 0; index < queue.length; index++) {
      if (queue[index].url == url) {
        return queue[index];
      }
    }
    for (index = 0; index < parallel.length; index++) {
      if (parallel[index].deferred.url == url) {
        return parallel[index].deferred;
      }
    }
    deferred = new ImpressDeferred(url, this.options);

    this.queue.push(deferred);
    this.parallel.digest();

    return deferred;
  },

  poll: function() {
    return this.queue.shift();
  },

  _registerPoolWatcher: function() {
    var
      queue = this,
      parallel = this.parallel;

    this.parallel.watch(function() {
      if (!parallel.hasLimit() && !queue.isEmpty()) {
        parallel.add(queue.poll());
        console.log('STATUS Queue size:', queue.queue.length, '>', 'Parallel:', parallel.parallel.length);
      }
      else {
        console.log('STATUS Queue size:', queue.queue.length, 'Parallel:', parallel.parallel.length);
      }
    });
  }

};
