...
Server.prototype = {
  ...
  _registerServerExtension: function() {
    this.instance.use(function(req, res, next) {
      ...
      getPerformTime = function() {
        var
          performHrTime = process.hrtime(hrTime);
        return (performHrTime[0] * 1e3 + performHrTime[1] * 1e-6).toFixed(3);
      };
      ...
      res.send = function(code, content) {
        ...
        res.statusCode = code;
        res.setHeader('X-Response-Time', getPerformTime()+'ms');
        res.end(content);
        console.log('Response Time:', getPerformTime(), 'ms');
      };
      next();
    });
  },
  ...
  _createQueue: function() {
    this.queue = new ImpressQueue(this.options);
    this.queue.init();
  }
