...
function Server(options, storage, htmlFilters) {
  ...
  this.proxy = this.options.proxy || typeof this.options.proxy == 'undefined';
  this.frontend = this.options.frontend || typeof this.options.frontend == 'undefined';
    ...
    this.instance.use(function(req, res, next) {
      ...
      if (!self.frontend) {
        if (parsedUrl.pathname != '/') {
          console.error('Unresolved path', parsedUrl.pathname);
          res.send(404);
          return;
        }
        force = /^\s*(on|yes|y|true)\s*/i.test(query.force || query['impress-force'] || query['impress_force']);
        ...
        force = /^\s*(on|yes|y|true)\s*/i.test(req.headers['x-impress-force'] || req.headers['impress-force']);
      }
